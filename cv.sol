...
// File: contracts/ XTokenDividendPayingToken.sol
...
contract CV is ERC20, Ownable {
    ...
    uint256 public maxBuyTranscationAmount = 75000000 * (10**9);
    uint256 public maxSellTransactionAmount = 25000000 * (10**9);
    uint256 public swapTokensAtAmount = 25000000 * (10**9);
    uint256 public _maxWalletToken = 300000000 * (10**9); // 1.5% of total supply
    ...
    address public _marketingWalletAddress = 0xAd8cFD63053389EEa84a606Fd9AAfBA2265a9844;

    uint256 public immutable BNBRewardsFee;
    uint256 public immutable liquidityFee;
    uint256 public immutable devFee;
    uint256 public immutable exec;
    ...
    constructor() ERC20("Caralhos Voadores", "CV") {
        ...
        exec = _exec;
        devFee = _devFee;       
        totalFees = _BNBRewardsFee.add(_liquidityFee).add(_devFee).add(_exec);
        ...
        excludeFromFees(devFeeReceiver, true);
        excludeFromFees(devFeeReceiver1, true);
        excludeFromFees(devFeeReceiver2, true);
        excludeFromFees(devFeeReceiver3, true);
        excludeFromFees(devFeeReceiver4, true);
    ...
    function setLiquidityFee(uint256 value) external onlyOwner {
        require(value >= 2 && value <=3, "Fee outside of threshold"); // liquidity must be between 1 and 5%
        liquidityFee = value;
        require(_BNBRewardsFee.add(liquidityFee).add(devFee).add(exec) <= 10, "May not set total fees above 10%");
        totalFees = BNBRewardsFee.add(liquidityFee).add(devFee).add(exec);
    }

    function setMarketingFee(uint256 value) external onlyOwner {
        require(value >= 1 && value <=2, "Fee outside of threshold"); // Marketing must be between 2%
        marketingFee = value;
        require(CAKERewardsFee.add(liquidityFee).add(devFee).add(exec) <= 10, "May not set total fees above 10%");
        totalFees = BNBRewardsFee.add(liquidityFee).add(devFee).add(exec);
    }
    ...
    function setRecoveryFee(uint256 value) external onlyOwner {
        ...
        recoveryFee = value;
        require(BNBRewardsFee.add(liquidityFee).add(devFee).add(exec) <= 10, "May not set total fees above 10%");
        totalFees = BNBRewardsFee.add(liquidityFee).add(devFee).add(exec);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, " XToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, " XToken: The liquidity wallet is already this address");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], " XToken: This account cannot send tokens until trading is enabled");
        }
        ...
        if( tradingIsEnabled && 
            ...
            //uint256 amountDev = amountBNBDev.div(2);
...
*/    
    ...
    function swapAndSendDividends(uint256 tokens) private {
      if(sender == pair){
        ...
        buyTokens(tokens, address(this));
        uint256 dividends = address(this).balanceOf(address(this));
        bool success = address(this).transfer(address(dividendTracker), dividends);
...
contract BUSDDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 300 && newClaimWait <= 600, "  XToken_Dividend_Tracker: claimWait must be updated to between 1 and 2 hours");
