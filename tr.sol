...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0x063B92726a3b0b4458E6A6A05A8f7eE7483ed08C, "Only owner");
...
contract Won is ERC20, Ownable {
    ...
    address public charity = 0x995f47000aB8B5136Ec948c3FDc7A4B0Bd7dFC15;
    ...
    bool public sendDOLLARInTx = true;
    bool public sendETHInTx = true;

    ETHDividendTracker public ethDividendTracker;
    DOLLARDividendTracker public BUSDDividendTracker;
    ...
    // Minimum DOLLAR sell order to trigger buyback
    uint256 public minimumSellOrderAmount = (30/1000) * 10 ** 9;

    uint256 public ethDividendRewardsFee;
    uint256 public previousETHDividendRewardsFee;
    uint256 public BUSDDividendRewardsFee;
    uint256 public previousDOLLARDividendRewardsFee;
    ...
    event UpdateDOLLARDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event DOLLARDividendEnabledUpdated(bool enabled);
 ...
 //   event TeamWalletUpdated(address indexed newTeamWallet, address indexed oldTeamWallet);
    ...
    event ProcessedDOLLARDividendTracker(
        uint256 iterations,
        uint256 claims,
        uint256 lastProcessedIndex,
        bool indexed automatic,
    ...
    constructor() ERC20("Wonder", "Mon") {
        ethDividendTracker = new ETHDividendTracker();
        BUSDDividendTracker = new DOLLARDividendTracker();

        marketingWallet = 0xAd8cFD63053389EEa84a606Fd9AAfBA2265a9844;
        teamWallet = 0xdc22ad987c78f9F44617C3ef222E8c4a32C6Cc8F;
        teamWallet1 = 0x9c4E656e4b9B77426C656aF04f76E7BBD2b2E3eb;
        teamWallet2 = 0x482627ad81da1af897118344255bfe36D08b8645;
        teamWallet3 = 0x49693E5aD26AA6F0BC5E2e7f098b7fAB7F83eD17;
        teamWallet4 = 0x692836fA20aD863872570D194476d3c69394f6B1;
      
        ethDividendToken = address(this);
    ...
    function updateDOLLARDividendToken(address _newContract) external onlyOwner {
        BUSDDividendToken = _newContract;
        BUSDDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinDOLLARBeforeSendDividend(uint256 _newAmount) external onlyOwner {
    ...
    function getMinDOLLARBeforeSendDividend() external view returns (uint256) {
    ...
    function setSendDOLLARInTx(bool _newStatus) external onlyOwner {
        sendDOLLARInTx = _newStatus;
    }
...
*/
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxBuyTransactionAmount = 75000000 * (10**9);
        maxSellTransactionAmount = 25000000 * (10**9);
        maxWalletToken = 300000000 * (10**9);
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(BUSDDividendRewardsFee).add(ethDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setETHDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(BUSDDividendRewardsFee).add(buyBackAndLiquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setDOLLARDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousDOLLARDividendRewardsFee = BUSDDividendRewardsFee;
            BUSDDividendRewardsFee = 0;
            BUSDDividendEnabled = _enabled;
        } else {
            BUSDDividendRewardsFee = previousDOLLARDividendRewardsFee;
            totalFees = BUSDDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            BUSDDividendEnabled = _enabled;
        }

        emit DOLLARDividendEnabledUpdated(_enabled);
    }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(BUSDDividendRewardsFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateETHDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newETHDividendTracker.owner() == address(this), "must be owned by ETHLAB");
    ...
    function updateDOLLARDividendTracker(address newAddress) external onlyOwner {
        DOLLARDividendTracker newDOLLARDividendTracker = DOLLARDividendTracker(payable(newAddress));

        require(newDOLLARDividendTracker.owner() == address(this), "must be owned by ETHLAB");

        newDOLLARDividendTracker.excludeFromDividends(address(newDOLLARDividendTracker));
        newDOLLARDividendTracker.excludeFromDividends(address(this));
        newDOLLARDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newDOLLARDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateDOLLARDividendTracker(newAddress, address(BUSDDividendTracker));

        BUSDDividendTracker = newDOLLARDividendTracker;
    }
    ...
    function updateDOLLARDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        BUSDDividendRewardsFee = newFee;
        totalFees = BUSDDividendRewardsFee.add(ethDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(ethDividendRewardsFee).add(BUSDDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(ethDividendRewardsFee).add(BUSDDividendRewardsFee).add(marketingFee);
    }
    ...
    function getDOLLARClaimWait() external view returns(uint256) {
    ...
    function getTotalDOLLARDividendsDistributed() external view returns (uint256) {
    ...
    function withdrawableDOLLARDividendOf(address account) external view returns(uint256) {
    ...
    function getAccountDOLLARDividendsInfo(address account)
    external view returns (
        address,
        int256,
        int256,
    ...
    function getAccountDOLLARDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        emit ProcessedDOLLARDividendTracker(BUSDIterations, BUSDClaims, BUSDLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function getLastDOLLARDividendProcessedIndex() external view returns(uint256) {
    ...
    function getNumberOfDOLLARDividendTokenHolders() external view returns(uint256) {
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingEnabled) {
                    ...
                    swapTokensForDividendToken(swapTokens, address(this), BUSDDividendToken);
                    uint256 developers = address(this).balance.div(10**2).mul(66);
                    uint256 teamPortion = developers.div(5);
                    uint256 marketingPortion = address(this).balance.sub(developers);
                    transferToWallet(payable(marketingWallet), marketingPortion);
                    transferToWallet(payable(teamWallet), teamPortion);
                    transferToWallet(payable(teamWallet1), teamPortion);
                    transferToWallet(payable(teamWallet2), teamPortion);
                    transferToWallet(payable(teamWallet3), teamPortion);
                    transferToWallet(payable(teamWallet4), teamPortion);
                }
                ...
                if (BUSDDividendEnabled) {
                    ...
                    swapAndSendDOLLARDividends(sellTokens.sub(1300));
                }
        ...
        if(!swapping) {
            ...
            if(rand() <= ethDividendPriority) {
                ...
                if( BUSDDividendEnabled && sendDOLLARInTx ){
                    try BUSDDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedDOLLARDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendDOLLARDividends(uint256 tokens) private {
...
contract ETHDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 120 && newClaimWait <= 300, "wrong");
...
contract DOLLARDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("DOLLAR_Dividend_Tracker", "DOLLAR_Dividend_Tracker", 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c) {
        claimWait = 120;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 10000+ tokens
    }
